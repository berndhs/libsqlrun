...
SqlRunner::SendSignals ()
{
  while (!signalList.isEmpty()) {
    ...
    case Sig_Result:
      emit Finished (queryMap[sig.value0], sig.okFlag);
      break;
    case Sig_Transaction:
      emit Transaction (dbaseMap[sig.value0], sig.okFlag);
      break;
    case Sig_Commit:
...
void
SqlRunner::RequestTransaction (int openId)
{
  RequestStruct req (Req_Transaction);
  req.openId = openId;
  requestLock.lock ();
  requestList.append (req);
  requestLock.unlock ();
...
void
SqlRunner::RequestCommit (int openId)
{
  RequestStruct req (Req_Commit);
  req.openId = openId;
  requestLock.lock ();
  requestList.append (req);
  requestLock.unlock ();
...
void
SqlRunner::DoTransaction (RequestStruct & req)
{
  bool ok (false);
  if (dbaseMap.contains (req.openId)) {
    SqlRunDatabase * db = dbaseMap[req.openId];
    if (db && db->sqlDB) {
      ok =db->sqlDB->transaction ();
    }
  }
  SignalStruct sig (Sig_Transaction);
  sig.value0 = req.openId;
  sig.okFlag = ok;
  signalList.append (sig);
}
...
void
SqlRunner::DoCommit (RequestStruct & req)
{
  bool ok (false);
  if (dbaseMap.contains (req.openId)) {
    SqlRunDatabase * db = dbaseMap[req.openId];
